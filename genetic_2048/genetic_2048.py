...
class Genetic2048:
    ...
    def create_genome(self):
        genome = {
            ...
            "avg_dist_between_largest": uniform(0, 1),
            "holes": uniform(0, 1),
            "spaces_cleared": uniform(0, 1),
            "points_earned": uniform(0, 1),
        }
        return genome
    ...
    def update(self):
        if self.current_genome != -1:
            current_grid = self.grid# self.get_grid()
    ...
    def get_all_possible_moves(self):
        ...
        move_funcs = [self.slide_up, self.slide_down, self.slide_left, self.slide_right]
        current_grid = self.grid # getgrid
        for m in range(len(moves)):
            ...
            rating += current_genome["eccentricity"] * algorithm["eccentricity"]
            ...
            rating += current_genome["spaces_cleared"] * algorithm["spaces_cleared"]
            rating += current_genome["points_earned"] * algorithm["points_earned"]
    ...
    def mutate(self, genome):
        ...
        for key in genome:
            if uniform(0, 1) < self.mutation_rate:
                new_genome[key]+= (self.mutation_step * uniform(0,1) * 2 - self.mutation_step)
        return new_genome
    ...
    def check_if_lost(self, matrix):
        lost = len(np.unique(matrix)) == len(matrix)*len(matrix[0])
        return lost
